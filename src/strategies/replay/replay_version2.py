...
class ReplayVersion2Strategy(Strategy, Debug):
    def __init__(self, agent, identifier, history):
        super(ReplayVersion2Strategy, self).__init__(agent)
        self.agent = agent
        ...
        self.last_waiting_time = self.next_order.creation_time - self.get_current_time()
    ...
    def get_current_time(self):
        return self.agent.get_current_time()
    ...
    def output_function(self, current_time, elapsed):
        ...
        for output_port in self.get_devs_model().output_ports:
            if output_port == 'out_order':
                send, order = self.get_devs_model().strategy.get_next_order(current_time, elapsed)
                if send is True:
    ...
    def process_in_next(self, current_time, elapsed, message):
        ...
        self.debug("%f" % self.next_wakeup_time)
